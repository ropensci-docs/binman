...
data.frames can then be passed to the `assign_directory` function.
...
Directories are assigned using the `rappdirs` package. See
[`rappdirs::user_data_dir`](https://rappdirs.r-lib.org/reference/user_data_dir.html)
for more OS specific information.

### Download Function

The download function should take as its primary input the output given
by `assign_directory` that is a named list of data.frames with column
names “version”, “url”, “file”, “dir” and “exists”.

The download function should process the list downloading the “url” to
the given directory “dir” assigning it the file-name “file”. Whether the
...
platform the file downloaded relates to. The “file” column gives the
full path to the downloaded file. The “processed” column indicates
whether the file was downloaded/processed.
...
The post-download function takes as its primary input the output given
by a “download function” that is a data.frame with column names
“platform”, “file” and “processed”.

The purpose of the post-download function is to process the downloaded
files after the download. This could involve changing the file
properties such as the mode, unzip/untar a downloaded zip/tar file etc.

### Application YAML file

The three functions mentioned previously are combined together to define
a process for downloading the binary file. The user stipulates the
required functions and arguments by way of a YAML format file. For
example:
...
The following is an example of using binman to get the github assets
from a project. The project is
...
<https://api.github.com/repos/lightbody/browsermob-proxy/releases>. We
shall breakdown the process into its three functional parts.
...
This gives us a list of 10 releases. A GitHub release should have a
version number.
...
The assets will contain the other information we need for our
pre-download function.
...
In the case of BMP we have a single platform which we could denote
“generic”. So we could pass the following:
...
In the `binman` package there is a function template for github asset
type downloads:
...
want to loop over the data.frame downloading the data at “url” to the
“file” in the “dir”. If it already “exists” we may not perform the
download.
...
So we simply need to pass the list from the pre download function to
this download function.

### BMP Post Download

The files we receive from the Browser mob Proxy project are zipped. As a
post processing operation we would like to unzip these files. The result
...
Again `binman` has a simple template function to unzip/untar a
downloaded file so we pass our output from the download to this:
...
Looking at the resulting directory structure (this is for Linux the
location of directories will be OS dependent):
